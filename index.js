const { DOMPurify, showdown } = window;

function substituteParams(value) {
    const profile = state?.preferences?.profile || {};
    const replacements = {
        '{{char}}': profile.characterName || '角色名',
        '{{user}}': profile.userName || '用户名',
        '{{date}}': new Intl.DateTimeFormat('zh-CN', { dateStyle: 'long' }).format(new Date()),
        '{{time}}': new Intl.DateTimeFormat('zh-CN', { timeStyle: 'short' }).format(new Date()),
    };
    return Object.entries(replacements).reduce((output, [macro, replacement]) => output.split(macro).join(replacement), String(value ?? ''));
}
...
const SYSTEM_FONT_FAMILY = 'var(--mainFontFamily)';
...
function defaultState() {
    return {
        ...
        templates: [],
        booklets: [],
        preferences: { lastView: 'library', readerTransition: 'simulation', builtInGalleryVersion: 0, profile: { userName: '', characterName: '' } },
    };
}
...
function resolvedFontFamily(fontFamily) {
    if (fontFamily !== SYSTEM_FONT_FAMILY) return fontFamily || 'serif';
    ...
    dialog.innerHTML = `
        <div class="wx-shell">
            <header class="wx-header">
                ...
                <button class="wx-icon-button" data-action="profile" title="本地称呼设置"><i class="fa-solid fa-user-pen"></i></button>
            </header>
...
function profileModal() {
    const profile = state.preferences.profile || {};
    showOverlay('本地设置', `<div class="wx-form-stack">
        <p class="wx-modal-intro">这些称呼只保存在当前浏览器，用于模板中的快捷文本。</p>
        <label>用户名<input id="wx-profile-user" value="${escapeHtml(profile.userName || '')}" placeholder="用于 {{user}}"></label>
        <label>角色名<input id="wx-profile-character" value="${escapeHtml(profile.characterName || '')}" placeholder="用于 {{char}}"></label>
        <div class="wx-data-tools"><div><strong>本地数据备份</strong><span>导出或恢复文库、图库、字体、模板与册子。</span></div><button class="wx-button wx-quiet" data-action="export-backup"><i class="fa-solid fa-download"></i> 导出</button><button class="wx-button wx-quiet" data-action="import-backup"><i class="fa-solid fa-upload"></i> 恢复</button></div>
    </div>`, `<button class="wx-button wx-quiet" data-action="close-overlay">取消</button><button class="wx-button" data-action="save-profile">保存设置</button>`);
}

function saveProfile() {
    state.preferences.profile = {
        userName: dialog.querySelector('#wx-profile-user')?.value.trim() || '',
        characterName: dialog.querySelector('#wx-profile-character')?.value.trim() || '',
    };
    scheduleSave();
    closeOverlay();
    notify('本地设置已保存。');
...
function exportBackup() {
    const payload = { format: 'wenxin-browser-backup', version: 1, exportedAt: new Date().toISOString(), state };
    downloadBlob(new Blob([JSON.stringify(payload, null, 2)], { type: 'application/json' }), `文心完整备份-${new Date().toISOString().slice(0, 10)}.json`);
    notify('完整备份已导出。');
}
...
function importBackup() {
    ...
    input.addEventListener('change', async () => {
        ...
        try {
            ...
            if (payload?.format !== 'wenxin-browser-backup' || !payload.state) throw new Error('不是有效的文心完整备份');
            state = { ...defaultState(), ...payload.state };
            state.preferences = { ...defaultState().preferences, ...(payload.state.preferences || {}) };
            for (const key of ['library', 'fonts', 'images', 'folders', 'tags', 'templates', 'booklets']) {
                if (!Array.isArray(state[key])) state[key] = [];
            }
            await persistState();
...
async function handleClick(event) {
    ...
    else if (action === 'profile') profileModal();
